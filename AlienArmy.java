...
public class AlienArmy extends Objects{
   ...
   public AlienArmy(int x, int y, Ship s, SpaceInvaders si, Image ai, Control control) {
       super(x,y);
	   ship = s;
   spaceInvaders = si;
...
//  alienImage = new javax.swing.ImageIcon("alienFull.jpg").getImage();
   ...
   /**
    * In this method we initialize the 3 rows of aliens
    * that make up the army.
    */
   private void createArmy() {
       //To initialize a row we can either do
   ...
   rowOne[9] = new Alien(alienImage, spaceInvaders);
       ...
       //Or more efficiently we could use a loop
   for (int i = 0; i < 10; i++) {
   ...
   int leftStart = 50;//Sets the furthest position to the left
   for (int i = 0; i < 10; i++) {
   ...
   int random = generator.nextInt(200);
		   if(random > 20 && random < 30){
   int rnd1 = generator.nextInt(10);
   int rnd2 = generator.nextInt(10);
   int rnd3 = generator.nextInt(10);
   if (!rowOne[rnd1].hasBeenHit()) {
       AlienShot as = new AlienShot(rowOne[rnd1].getXPos()+(int)(Alien.ALIEN_WIDTH/2), rowOne[rnd1].getYPos(),control,ship);
       control.addObject(as);
//       System.out.println("!");
       //   AlienShot.drawShot();
       //   AlienShot.tick();
   ...
   if (!rowOne[rnd2].hasBeenHit()) {  
       AlienShot as = new AlienShot(rowTwo[rnd2].getXPos()+(int)(Alien.ALIEN_WIDTH/2), rowTwo[rnd2].getYPos(),control, ship);
       control.addObject(as);
         alienShots.addElement(as);
       //  System.out.println("@");
   }
   if (!rowOne[rnd3].hasBeenHit()) {  
           AlienShot as = new AlienShot(rowThree[rnd3].getXPos()+(int)(Alien.ALIEN_WIDTH/2), rowThree[rnd3].getYPos(), control,ship);
           control.addObject(as);
           alienShots.addElement(as);
        //   System.out.println("#");
   }
...
public void checkDead() {
	if(rowOne.length == 0 &&rowTwo.length == 0 && rowThree.length == 0  ) {
		spaceInvaders.state =4;
	}
}
