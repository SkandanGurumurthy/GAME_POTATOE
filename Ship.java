...
public class Ship implements MouseListener, MouseMotionListener {
    ...
    *
    */
   public Ship(SpaceInvaders si, Control control) {
       spaceInvaders = si;
   ...
   public void mouseClicked(MouseEvent me) {
       ...
       shot = new Shot(x+(int)(SHIP_WIDTH/2), 360,control, army);
       control.addObject(shot);
     //  control.object.add(shot);
   ...
   public boolean checkShot(int xShot, int yShot) {
       ...
       //First let's check the X range
       if ((xShot >= x) && (xShot <= (x+SHIP_WIDTH))) {
           //X is ok, now let's check the Y range
